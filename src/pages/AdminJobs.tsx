...
export default function AdminJobs() {
  ...
  const handleBulkDelete = async () => {
    ...
    const { error } = await supabase.from('jobs').update({ deleted_at: new Date().toISOString() }).in('id', ids)
    if (error) {
      toastError('فشل في حذف الوظائف')
    } else {
      success(`تم حذف ${selectedJobs.size} وظيفة بنجاح`)
  ...
  const handleToggleStatus = async (id: string, currentStatus: string) => {
    ...
    if (error) {
      toastError('فشل في تحديث حالة الوظيفة')
    } else {
      success(newStatus === 'active' ? 'تم تفعيل الوظيفة' : 'تم تعطيل الوظيفة')
      fetchJobs()
    }
  }

  const handleToggleFeatured = async (id: string, currentFeatured: boolean) => {
    const { error } = await supabase.from('jobs').update({ is_featured: !currentFeatured }).eq('id', id)
    if (error) {
      toastError('فشل في تحديث الوظيفة')
    } else {
      success(!currentFeatured ? 'تم تمييز الوظيفة' : 'تم إزالة التمييز')
      fetchJobs()
    }
  }
