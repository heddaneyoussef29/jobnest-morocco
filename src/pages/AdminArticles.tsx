...
export default function AdminArticles() {
  ...
  const handleBulkDelete = async () => {
    ...
    const { error } = await supabase.from('articles').update({ deleted_at: new Date().toISOString() }).in('id', ids)
    if (error) {
      toastError('فشل في حذف المقالات')
    } else {
      success(`تم حذف ${selectedArticles.size} مقال بنجاح`)
  ...
  const handlePublish = async (id: string, currentStatus: string) => {
    ...
    if (error) {
      toastError('فشل في تحديث حالة المقال')
    } else {
      success(newStatus === 'published' ? 'تم نشر المقال بنجاح' : 'تم إلغاء نشر المقال')
      fetchArticles()
    }
  }
