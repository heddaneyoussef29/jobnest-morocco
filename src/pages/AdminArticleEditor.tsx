...
interface NavPage { id: string; title: string; slug: string; pageType?: string }
...
export default function AdminArticleEditor() {
  ...
  const [navPageSlug, setNavPageSlug] = useState<string>('')
  ...
  useEffect(() => { fetchCategories(); fetchNavPages(); if (isEditing && id) fetchArticle(id) }, [id])
  ...
  const fetchNavPages = async () => {
    ...
    if (data?.value) {
      try {
        ...
        // استخراج الصفحات من نوع 'articles' فقط (أو جميع الصفحات)
        const pages: NavPage[] = []
        const extractPages = (items: any[]) => {
          for (const item of items) {
            pages.push({ id: item.id, title: item.title, slug: item.slug, pageType: item.pageType || 'static' })
            if (item.subItems?.length) extractPages(item.subItems)
          }
        }
        extractPages(items)
        setNavPages(pages)
  ...
  const fetchArticle = async (articleId: string) => {
    ...
    if (data) {
      ...
      setSearchKeywords(data.search_keywords || ''); setNavPageSlug(data.nav_page_slug || '')
    }
  }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch (err: any) { toastError(err.message || 'خطأ في رفع الصورة') }
  ...
  const handleSave = async (publishStatus: 'draft' | 'published') => {
    ...
    const articleData = {
      ...
      category_id: categoryId || null, nav_page_slug: navPageSlug || null, status: publishStatus,
    ...
    try {
      ...
      success(publishStatus === 'published' ? (isEditing ? 'تم تحديث ونشر المقال بنجاح' : 'تم نشر المقال بنجاح') : 'تم حفظ المسودة بنجاح')
      setTimeout(() => navigate('/admin/articles'), 800)
    } catch (err: any) { toastError(err.message || 'خطأ في الحفظ') }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">📄 الصفحة في التنقل</h3>
              <p className="text-xs text-gray-500 mb-3">اختر الصفحة التي يظهر فيها هذا المقال</p>
              <select value={navPageSlug} onChange={(e) => setNavPageSlug(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl outline-none bg-gray-50 focus:bg-white">
                <option value="">بدون صفحة محددة</option>
                {navPages.filter(p => p.pageType === 'articles').map((page) => (
                  <option key={page.id} value={page.slug}>{page.title}</option>
                ))}
              </select>
              {navPages.filter(p => p.pageType === 'articles').length === 0 && (
                <p className="text-xs text-amber-600 mt-2">💡 أنشئ صفحة من نوع "مقالات" في إدارة التنقل أولاً</p>
              )}
            </div>
